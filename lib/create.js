// Generated by CoffeeScript 2.5.1
(function() {
  var CWD, DIR, YAML, createIcons, createManifest, createPackageFile, createPages, createScripts, createSrc, createStyles, createWorkers, exec, fs;

  fs = require('fs');

  ({exec} = require('child_process'));

  YAML = require('yaml');

  CWD = process.cwd();

  DIR = "/tmp/makepwa/path";

  exports.create = function(name) {
    var spec;
    DIR = `${CWD}/${name}`;
    if (fs.existsSync(DIR)) {
      console.error(`${DIR} already exists.`);
      process.exit(1);
    } else {
      fs.mkdirSync(DIR);
    }
    spec = {
      name: name,
      version: '0.0.0',
      devDependencies: {
        coffeescript: "^2.5.1",
        'coffee-loader': "^1.0.1"
      }
    };
    createPackageFile(spec);
    createSrc();
    return exec('npm install', {
      cwd: DIR
    });
  };

  createPackageFile = function(spec) {
    var source;
    source = JSON.stringify(spec, null, 2);
    return fs.writeFileSync(`${DIR}/package.json`, source);
  };

  createSrc = function() {
    var src;
    src = `${DIR}/src`;
    fs.mkdirSync(src);
    createPages(src);
    createStyles(src);
    createScripts(src);
    createWorkers(src);
    createIcons(src);
    return createManifest(src);
  };

  createPages = function(src) {
    var dir, source;
    dir = `${src}/pages`;
    fs.mkdirSync(dir);
    source = `doctype html
html
  head
    title Title

    meta(charset="utf-8")
    meta(http-equiv="x-ua-compatible" content="ie=edge")
    meta(name="viewport" content="width=device-width, initial-scale=1.0")

    link(rel="manifest" href="/manifest.webmanifest")
    link(rel="stylesheet" href="/styles/main.css")
    script(src="/scripts/main.js")
  body
    #app`;
    return fs.writeFileSync(`${dir}/index.pug`, source);
  };

  createStyles = function(src) {
    var dir, source;
    dir = `${src}/styles`;
    fs.mkdirSync(dir);
    source = `body
  background-color: black`;
    return fs.writeFileSync(`${dir}/main.sass`, source);
  };

  createScripts = function(src) {
    var dir;
    dir = `${src}/scripts`;
    fs.mkdirSync(dir);
    fs.writeFileSync(`${dir}/main.coffee`, `require './register_service_worker.coffee'

console.log 'from main'`);
    return fs.writeFileSync(`${dir}/register_service_worker.coffee`, `unless 'serviceWorker' in navigator
  navigator.serviceWorker.register('/sw.js')
    .then (registration) ->
      console.log 'Service worker registration succeeded:', registration
    .catch (error) ->
      console.log 'Service worker registration failed:', error
else
  console.log 'Service workers are not supported.'`);
  };

  createWorkers = function(src) {
    var dir;
    dir = `${src}/workers`;
    fs.mkdirSync(dir);
    return fs.writeFileSync(`${dir}/sw.coffee`, `self.onactivate = (event) ->
  console.log 'from onactivate'
  event.waitUntil Promise.resolve()

self.oninstall = (event) ->
  console.log 'from oninstall'
  event.waitUntil Promise.resolve()

self.onfetch = (event) ->
  console.log "Logging an HTTP request from a service worker:"
  console.log event.request
  event.respondWith fetch event.request`);
  };

  createIcons = function(src) {
    var dir;
    dir = `${src}/icons`;
    fs.mkdirSync(dir);
    return fs.copyFile(`${__dirname}/create/icon.192x192.png`, `${src}/icons/icon.192x192.png`, function(error) {
      if (error) {
        throw error;
      }
    });
  };

  createManifest = function(src) {
    var icon192, spec;
    icon192 = {
      src: '/icons/icon.192x192.png',
      sizes: '192x192',
      type: 'image/png'
    };
    spec = {
      name: 'makepwa0',
      short_name: 'makepwa0',
      description: 'Some description',
      start_url: '/index.html',
      display: 'fullscreen',
      background_color: 'black',
      icons: [icon192]
    };
    return fs.writeFileSync(`${src}/manifest.yml`, YAML.stringify(spec));
  };

}).call(this);
